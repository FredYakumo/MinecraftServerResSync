...
std::unordered_map<std::string, std::string> res_manage::get_path_file_list(const std::string_view base_path) {
    ...
    for (const auto &entry : fs::directory_iterator(bp)) {
        if (entry.is_directory()) {
            ...
            debug("Look up file path: {}", p);
            for (auto path_list = get_path_file_list(p.c_str()); const auto &[fst, snd] : path_list) {
                ret.insert({fst, snd});
            }
        } else {
            const auto &p = entry.path().string();
            debug("Calculate file's sha1: {}", p);
            std::ifstream f{p};
...
models::ClassFileResource res_manage::fetch_file_hash_map_from_managed_res(const models::ManageClassPathMap &managed_res,
    ...
    for (const auto &[fst, snd] : managed_res.managed_class_map) {
        ...
        for (const auto &path : snd.path_list) {
            for (decltype(auto) file_hash_list = get_path_file_list(path.c_str()); const auto &[fst, snd] : file_hash_list) {
