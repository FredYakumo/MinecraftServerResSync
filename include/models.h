...
public:
    ...
    [[nodiscard]] inline const std::unordered_map<std::string,
                                                  const std::string> &
    file_hash_map() const {
...
public:
    ...
    [[nodiscard]] const std::unordered_map<std::string, FileHashMap> &
    class_file_hash_map() const {
        return m_class_file_hash_map;
    }

    ClassFileResource &insert_file_hash_map(std::string_view class_name,
...
public:
    ...
    explicit ServerData(
        ...
        const std::string_view host, const uint64_t listen_port,
        const uint16_t thread_count = 4)
        : m_class_file_resources(std::move(class_file_resource)), m_host(host),
    ...
    explicit ServerData(ClassFileResource class_file_resource,
        ...
        : m_class_file_resources(
          ...
          m_host(host), m_listen_port(listen_port),
