...
namespace components {
    ...
    void write_file_content_to_response(const std::string_view file_name, response &response) {
        ...
        if (!fs.is_open()) {
            ...
            http_service::write_json_result(status::internal_server_error, json::value{{"message", "Internal Service error"}}, response);
            return;
        }

        response.result(status::ok);
        response.set(field::content_disposition, fmt::format("attachment; filename=\"{}\"", file_name));
    ...
    void get_resource_list(const request &req, response &response, const std::unordered_map<std::string_view, std::string_view> &param_map) {
        ...
        for (const auto &[fst, snd] : g_server_data.get_const()->class_file_resource()->class_file_hash_map()) {
            ...
            ret.push_back(json::value{{"class_name", fst}, {"file_list", file_list}});
        }

        http_service::write_json_result(status::ok, ret, response);
    }
    ...
    void fetch_resource(const request &req, response &response, const std::unordered_map<std::string_view, std::string_view> &param_map) {
        ...
        if (file_key == std::cend(param_map)) {
            http_service::write_json_result(status::bad_request, json::value{{"message", "file name param not provide"}}, response);
            return;
        }
        const auto class_key = param_map.find("class");
        if (class_key == std::cend(param_map)) {
            http_service::write_json_result(status::bad_request, json::value{{"message", "class name param not provide"}}, response);
            return;
        }
        ...
        if (class_map == std::cend(class_file_hash_map)) {
            http_service::write_json_result(status::not_found, json::value{{"message", fmt::format("Class name '{}' not found.", class_name)}}, response);
            return;
        }
        ...
        if (const auto &file_iter = class_map->second.file_hash_map().find(file_name); file_iter == std::cend(class_map->second.file_hash_map())) {
            http_service::write_json_result(status::not_found, json::value{{"message", fmt::format("File {} is not in class {}", file_name, class_name)}}, response);
            return;
        }
